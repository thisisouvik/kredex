...
export function ProfileSettingsForm({
  ...
  const emailLocked = Boolean(initialEmailUpdatedAt && (Date.now() - new Date(initialEmailUpdatedAt).getTime() < 24 * 60 * 60 * 1000));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccess(true);
      setUploadProgress(null);
      showAlert("Success", "Profile details saved successfully.", "success", 4000);
    ...
    <form className="settings-form-group" onSubmit={handleSubmit}>
      ...
      <div className="settings-field settings-field--full">
        ...
        <p className="settings-help-text" style={{ marginTop: "0.25rem" }}>
          You can only update your email address once every 24 hours for security reasons.
        </p>
      </div>
